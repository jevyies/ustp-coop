		...
		function($filter, $q, uiGridExporterConstants, gridUtil, $compile, $interval, i18nService) {
			var service = {
				...
				defaultGridOptions: function(gridOptions) {
					 ...
					 */
					gridOptions.exporterPdfFilename = gridOptions.exporterPdfFilename
						? gridOptions.exporterPdfFilename
						: 'download.pdf';
					/**
					 * @ngdoc object
					 ...
					 */
					gridOptions.exporterExcelFilename = gridOptions.exporterExcelFilename
					 ...
					 */
					gridOptions.exporterExcelSheetName = gridOptions.exporterExcelSheetName
						? gridOptions.exporterExcelSheetName
						: 'Sheet1';
					 ...
					 * <br/>Defaults to 200
					 */
					gridOptions.exporterMenuItemOrder = gridOptions.exporterMenuItemOrder
						? gridOptions.exporterMenuItemOrder
						: 200;
					/**
					 * @ngdoc object
					...
					gridOptions.exporterPdfTableStyle = gridOptions.exporterPdfTableStyle
						? gridOptions.exporterPdfTableStyle
						: { margin: [0, 5, 0, 15] };
					/**
					 * @ngdoc object
					...
					gridOptions.exporterPdfOrientation = gridOptions.exporterPdfOrientation
						? gridOptions.exporterPdfOrientation
						: 'landscape';
					/**
					 * @ngdoc object
					...
					gridOptions.exporterPdfPageSize = gridOptions.exporterPdfPageSize
						? gridOptions.exporterPdfPageSize
						: 'A4';
					/**
					 * @ngdoc object
					...
					gridOptions.exporterPdfMaxGridWidth = gridOptions.exporterPdfMaxGridWidth
						? gridOptions.exporterPdfMaxGridWidth
						: 720;
					/**
					 * @ngdoc object
					 ...
					 */
					gridOptions.exporterMenuAllData =
						gridOptions.exporterMenuAllData !== undefined ? gridOptions.exporterMenuAllData : true;
					...
					gridOptions.exporterMenuVisibleData =
						gridOptions.exporterMenuVisibleData !== undefined ? gridOptions.exporterMenuVisibleData : true;
					...
					gridOptions.exporterMenuSelectedData =
						gridOptions.exporterMenuSelectedData !== undefined
							? gridOptions.exporterMenuSelectedData
							: true;
					 ...
					 */
					gridOptions.exporterMenuCsv =
						gridOptions.exporterMenuCsv !== undefined ? gridOptions.exporterMenuCsv : true;
					 ...
					 */
					gridOptions.exporterMenuPdf =
						gridOptions.exporterMenuPdf !== undefined ? gridOptions.exporterMenuPdf : true;
					...
					gridOptions.exporterMenuExcel =
						gridOptions.exporterMenuExcel !== undefined ? gridOptions.exporterMenuExcel : true;
					 ...
					 * </pre>
					 */
					gridOptions.exporterPdfCustomFormatter =
						gridOptions.exporterPdfCustomFormatter &&
						typeof gridOptions.exporterPdfCustomFormatter === 'function'
							? gridOptions.exporterPdfCustomFormatter
							: function(docDef) {
									return docDef;
							  };
					 ...
					 * </pre>
					 */
					gridOptions.exporterFieldFormatCallback = gridOptions.exporterFieldFormatCallback
						? gridOptions.exporterFieldFormatCallback
						: function(grid, row, col, value) {
								return null;
						  };
					 ...
					 * </pre>
					 */
					gridOptions.exporterExcelCustomFormatters = gridOptions.exporterExcelCustomFormatters
						? gridOptions.exporterExcelCustomFormatters
						: function(grid, workbook, docDefinition) {
								return docDefinition;
						  };
					 ...
					 * </pre>
					 */
					gridOptions.exporterExcelHeader = gridOptions.exporterExcelHeader
					 ...
					 * <br/>Defaults to 3.5
					 */
					gridOptions.exporterColumnScaleFactor = gridOptions.exporterColumnScaleFactor
						? gridOptions.exporterColumnScaleFactor
						: 3.5;
					 ...
					 * </pre>
					 */
					gridOptions.exporterAllDataFn = gridOptions.exporterAllDataFn
						? gridOptions.exporterAllDataFn
						: null;
				...
				addToMenu: function(grid) {
					grid.api.core.addToGridMenu(grid, [
						...
						},
						{
							...
							shown: function() {
								return (
									grid.options.exporterMenuCsv &&
									grid.options.exporterMenuSelectedData &&
						...
						},
						{
							...
							shown: function() {
								return (
									grid.options.exporterMenuExcel &&
									grid.options.exporterMenuSelectedData &&
									(grid.api.selection && grid.api.selection.getSelectedRows().length > 0)
								);
							},
				...
				csvExport: function(grid, rowTypes, colTypes) {
					...
					this.loadAllDataIfNeeded(grid, rowTypes, colTypes).then(function() {
						...
						var csvContent = self.formatAsCsv(
							exportColumnHeaders,
							exportData,
						...
						self.downloadFile(
							...
							grid.options.exporterIsExcelCompatible
						);
					});
				},
				...
				loadAllDataIfNeeded: function(grid, rowTypes, colTypes) {
					if (
						...
						grid.rows.length !== grid.options.totalItems &&
						grid.options.exporterAllDataFn
					) {
						return grid.options.exporterAllDataFn().then(function(allData) {
				...
				getColumnHeaders: function(grid, colTypes) {
					...
					if (colTypes === uiGridExporterConstants.ALL) {
						...
						var leftColumns = grid.renderContainers.left
								? grid.renderContainers.left.visibleColumnCache.filter(function(column) {
										return column.visible;
								  })
								: [],
							bodyColumns = grid.renderContainers.body
								? grid.renderContainers.body.visibleColumnCache.filter(function(column) {
										return column.visible;
								  })
								: [],
							rightColumns = grid.renderContainers.right
								? grid.renderContainers.right.visibleColumnCache.filter(function(column) {
										return column.visible;
								  })
								: [];
					...
					columns.forEach(function(gridCol) {
						...
						if (
							...
							var headerEntry = {
								...
								align: gridCol.colDef.align
									? gridCol.colDef.align
									: gridCol.colDef.type === 'number'
									? 'right'
									: 'left',
							};
					...
					rows.forEach(function(row) {
						if (row.exporterEnableExporting !== false) {
							...
							columns.forEach(function(gridCol) {
								...
								if (
									(gridCol.visible || colTypes === uiGridExporterConstants.ALL) &&
									gridCol.colDef.exporterSuppressExport !== true &&
									gridCol.field !== '$$hashKey' &&
									grid.options.exporterSuppressColumns.indexOf(gridCol.name) === -1
								) {
									var cellValue = applyCellFilters
											? grid.getCellDisplayValue(row, gridCol)
											: grid.getCellValue(row, gridCol),
										extractedField = {
											value: grid.options.exporterFieldCallback(grid, row, gridCol, cellValue),
										},
										extension = grid.options.exporterFieldFormatCallback(
											grid,
											row,
											gridCol,
											cellValue
				...
				formatAsCsv: function(exportColumnHeaders, exportData, separator) {
					var self = this,
						bareHeaders = exportColumnHeaders.map(function(header) {
				...
				downloadFile: function(
					...
					if (navigator.msSaveBlob) {
						return navigator.msSaveOrOpenBlob(
							new Blob([exporterOlderExcelCompatibility ? '\uFEFF' : '', csvContent], {
								type: strMimeType,
							}),
							fileName
						);
				...
				calculatePdfHeaderWidths: function(grid, exportHeaders) {
					...
					return exportHeaders.map(function(header) {
						return header.width === '*'
							? header.width
							: (header.width * grid.options.exporterPdfMaxGridWidth) / gridWidth;
					});
				},
				...
				formatFieldAsPdfString: function(field) {
					...
					if (field.value == null) {
						...
						returnVal = JSON.stringify(field.value)
							.replace(/^"/, '')
							.replace(/"$/, '');
				...
				formatAsExcel: function(exportColumnHeaders, exportData, workbook, sheet, docDefinition) {
					...
					for (var i = 0; i < bareHeaders.length; i++) {
						...
						var metadata =
							docDefinition.styles && docDefinition.styles[exportStyle]
								? { style: docDefinition.styles[exportStyle].id }
								: null;
				...
				prepareAsExcel: function(grid, workbook, sheet) {
					...
					if (grid.options.exporterExcelHeader) {
						if (angular.isFunction(grid.options.exporterExcelHeader)) {
							...
							sheet.data.push([
								{ value: headerText, metadata: { style: docDefinition.styles[style].id } },
							]);
						}
					}
				...
				excelExport: function(grid, rowTypes, colTypes) {
					...
					this.loadAllDataIfNeeded(grid, rowTypes, colTypes).then(function() {
						...
						var aName = grid.options.exporterExcelSheetName
							? grid.options.exporterExcelSheetName
							: 'Sheet1';
						...
						var colWidths = [];
						var startDataIndex = grid.treeBase
							? grid.treeBase.numberLevels
							: grid.enableRowSelection
							? 1
							: 0;
						for (var i = startDataIndex; i < grid.columns.length; i++) {
							if (
								grid.columns[i].field !== uiGridExporterConstants.rowHeaderColName &&
								grid.columns[i].field !== uiGridExporterConstants.selectionRowHeaderColName
							) {
								colWidths.push({
									width: grid.columns[i].drawnWidth / grid.options.exporterColumnScaleFactor,
								});
							}
						}
						...
						var excelContent = self.formatAsExcel(
							exportColumnHeaders,
							exportData,
							workbook,
							sheet,
						...
						ExcelBuilder.Builder.createFile(workbook, { type: 'blob' }).then(function(result) {
							self.downloadFile(
								grid.options.exporterExcelFilename,
								result,
								grid.options.exporterCsvColumnSeparator,
								grid.options.exporterOlderExcelCompatibility
							);
